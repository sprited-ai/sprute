...
Reads GEMINI_API_KEY from sunny-street/.env.local unless set in the env.
Mirrors sunny-street's cam2portrait call shape: composite template image +
terse layout prompt, image part back out.
...
ENV_FALLBACK = "/Users/jin/dev/sunny-street/.env.local"
...
def api_key() -> str:
    ...
    for line in Path(ENV_FALLBACK).read_text().splitlines():
        if line.startswith("GEMINI_API_KEY="):
            return line.split("=", 1)[1].strip().strip('"')
    sys.exit("no GEMINI_API_KEY in env or sunny-street/.env.local")
