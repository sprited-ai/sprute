/** Model round-trips: composed template in → filled sheet out, plus the
 * self-review fix pass and the observational QC check. Pure fetch + RawImage
 * logic — platform specifics (PNG codec, label rendering, where the API key
 * comes from) enter through GenContext, so Node and the browser share this
 * file verbatim. */
import { crop, createImage, paste, scaleNearest, type RawImage } from "./image.js";
import { keyCell } from "./keyer.js";
...
export interface GenContext {
  codec: Codec;
  /** Resolve an API key by its env-style name (GEMINI_API_KEY, ...). Node
   * reads the environment / .env files; a browser app returns the key the
   * user typed in. */
  apiKey(envKey: string): string;
}
...
export async function generateSheet(ctx: GenContext, template: RawImage, prompt: string, opts: GenerateOptions = {}): Promise<RawImage> {
  ...
  if (provider === "gemini") {
    ...
    for (const part of json.candidates?.[0]?.content?.parts ?? []) {
      ...
      if (d) return ctx.codec.decodeImage(fromBase64(d.data));
    }
  ...
  if (provider === "novita-seedream") {
    ...
    return ctx.codec.decodeImage(new Uint8Array(await (await fetch(images[0])).arrayBuffer()));
  }
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (status === "TASK_STATUS_SUCCEED") {
      return ctx.codec.decodeImage(new Uint8Array(await (await fetch(json.images[0].image_url)).arrayBuffer()));
    }
...
export const FACINGS = ["front", "front-right", "right", "back-right", "back", "back-left", "left", "front-left"] as const;
...
export async function checkSpritesheet(ctx: GenContext, sheet: RawImage, description?: string): Promise<SheetCheck> {
  const key = ctx.apiKey("GEMINI_API_KEY");
  const b64 = toBase64(await ctx.codec.encodePng(sheet));
 ...
 * Mid-gray 128,128,128 background — the neutral gray the model already knows
 * from the template's sprite panels, and translucent parts stay visible
 * against it. */
export async function buildFixGrid(ctx: GenContext, cells: RawImage[], barH: number): Promise<Uint8Array> {
  const { width: cw, height: ch } = cells[0];
  const slotH = ch + barH;
  const grid = createImage(cw * 3, slotH * 3, [128, 128, 128, 255]);
  for (let r = 0; r < 3; r++) {
    for (let c = 0; c < 3; c++) {
      const slot = FIX_GRID[r][c];
      if (slot === null) continue;
      paste(grid, cells[slot], c * cw, r * slotH);
      paste(grid, await ctx.codec.drawLabel(FACINGS[slot], cw, barH), c * cw, r * slotH + ch);
    }
  }
  return ctx.codec.encodePng(grid);
}
...
export async function fixSpritesheet(
  ctx: GenContext, cells: RawImage[], description?: string, opts: GenerateOptions = {},
): Promise<{ cells: RawImage[]; report?: string; clean: boolean; gridPng: Uint8Array; raw?: RawImage }> {
  const model = opts.model ?? DEFAULT_MODEL.gemini;
  const key = ctx.apiKey(opts.envKey ?? "GEMINI_API_KEY");
  const { width: cw, height: ch } = cells[0];
  const barH = Math.max(24, Math.round(ch * 0.1));
  const gridPng = await buildFixGrid(ctx, cells, barH);
  const b64 = toBase64(gridPng);
  const prompt =
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    for (const part of json.candidates?.[0]?.content?.parts ?? []) {
      const d = part.inlineData ?? part.inline_data;
      if (d) image = await ctx.codec.decodeImage(fromBase64(d.data));
